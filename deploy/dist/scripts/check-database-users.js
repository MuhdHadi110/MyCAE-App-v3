...
function formatTable(header, rows) {
    const colWidths = header.map((_, colIndex) => {
        const maxWidth = Math.max(header[colIndex].length, ...rows.map(row => row[colIndex]?.length || 0));
        return maxWidth + 2;
    });
...
async function checkTestUsers(userRepo) {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    if (foundTestUsers.length === 0) {
        ...
        const rows = foundTestUsers.map((user) => [
            user.name,
            user.email,
...
async function checkFirstTimeUsers(userRepo) {
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('📋 USERS WHO HAVEN\'T LOGGED IN YET');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    // Use raw SQL since is_first_login column not in entity
    const unloggedInUsers = await userRepo.query(`SELECT id, name, email, roles, department, position, is_first_login, created_at
     FROM users
    ...
    if (unloggedInUsers.length === 0) {
        ...
        const header = ['Name', 'Email', 'Role', 'Department', 'Created'];
        const rows = unloggedInUsers.map((user) => {
            let roles = '-';
            try {
...
async function showSummary(userRepo, testUserCount, firstTimeUserCount) {
...
main();
//# sourceMappingURL=check-database-users.js.map