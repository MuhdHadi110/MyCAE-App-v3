...
router.post('/:id/submit-for-approval', (0, auth_1.authorize)(User_1.UserRole.SENIOR_ENGINEER, User_1.UserRole.PRINCIPAL_ENGINEER, User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    try {
        ...
        const mds = await userRepo.find({
            where: {
                roles: (0, typeorm_1.Raw)((alias) => `JSON_CONTAINS(${alias}.roles, '"managing-director"')`)
            }
        });
        // Format date/time
        ...
        await email_service_1.default.sendInvoiceApprovalRequest(invoice.id, mds, invoice.invoice_number, invoice.amount, invoice.currency, invoice.project_code, invoice.project_name, req.user.name, req.user.email, submittedDate, submittedTime).catch(err => {
...
router.get('/:id/pdf', async (req, res) => {
    try {
        ...
        if (invoice.file_url) {
            ...
            const filename = path_1.default.basename(invoice.file_url);
