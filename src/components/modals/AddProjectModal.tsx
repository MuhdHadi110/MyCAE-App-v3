...
export const AddProjectModal: React.FC<AddProjectModalProps> = ({ isOpen, onClose }) => {
  ...
  const projectManagers = teamMembers.filter(
    (tm) => {
      ...
      (role === 'senior-engineer' || role === 'principal-engineer' || role === 'manager' || role === 'managing-director');
    }
  );
                    ...
                    <div>
                      ...
                      {formData.isStructureContainer && (
                        <span className="text-xs text-amber-700 dark:text-amber-400 block mt-2">
                          ℹ️ Master folder projects cannot have direct POs. Create sub folders after project is created.
                        </span>
                      )}
                    </div>
                  </label>
