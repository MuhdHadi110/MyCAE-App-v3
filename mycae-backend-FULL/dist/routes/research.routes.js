        ...
        if (projectIds.length > 0) {
           ...
           WHERE table_schema = DATABASE() AND table_name = 'research_timesheets'`);
                ...
                if (tableExists[0]?.count > 0) {
                    ...
                    const hoursQuery = await database_1.AppDataSource.query(`SELECT research_project_id as projectId, SUM(hours) as totalHours 
             FROM research_timesheets 
             WHERE research_project_id IN (${placeholders}) 
             GROUP BY research_project_id`, projectIds);
                    ...
                    // Also check raw data
                    const rawData = await database_1.AppDataSource.query(`SELECT * FROM research_timesheets WHERE research_project_id IN (${placeholders})`, projectIds);
...
router.get('/summary', async (req, res) => {
    ...
    `;
        ...
        const rows = await database_1.AppDataSource.query(query, engineerId ? [engineerId] : []);
        ...
        const countResult = await database_1.AppDataSource.query(countQuery, engineerId ? [engineerId] : []);
...
router.get('/timesheets', async (req, res) => {
    ...
    `;
        const params = [];
        if (projectId) {
            query += ` AND rt.research_project_id = ?`;
            params.push(projectId);
        }
        if (teamMemberId) {
            query += ` AND rt.engineer_id = ?`;
        ...
        const timesheets = await database_1.AppDataSource.query(query, params);
        res.json(timesheets || []);
    }
    catch (error) {
...
router.delete('/projects/:id', async (req, res) => {
    try {
        ...
        await database_1.AppDataSource.query(`DELETE FROM research_timesheets WHERE research_project_id = ?`, [id]);
        // Then delete project
...
router.post('/timesheets', async (req, res) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
        await database_1.AppDataSource.query(query, [
            id,
            projectId,
            engineerId,
            date,
...
router.put('/timesheets/:id/approve', async (req, res) => {
    try {
      ...
      WHERE id = ?
    `;
        await database_1.AppDataSource.query(query, [id]);
...
router.delete('/timesheets/:id', async (req, res) => {
    try {
        const { id } = req.params;
        // Delete timesheet entry
        await database_1.AppDataSource.query(`DELETE FROM research_timesheets WHERE id = ?`, [id]);
