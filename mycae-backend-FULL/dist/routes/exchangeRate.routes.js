...
const exchangeRateApi_service_1 = require("../services/exchangeRateApi.service");
 ...
 */
router.post('/', (0, auth_1.authorize)(User_1.UserRole.SENIOR_ENGINEER, User_1.UserRole.PRINCIPAL_ENGINEER, User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    try {
        ...
        const exchangeRate = await currency_service_1.CurrencyService.setExchangeRate(fromCurrency.toUpperCase(), parseFloat(rate), new Date(effectiveDate), 'manual');
 ...
 */
router.post('/import', (0, auth_1.authorize)(User_1.UserRole.SENIOR_ENGINEER, User_1.UserRole.PRINCIPAL_ENGINEER, User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    try {
...
router.get('/single', async (req, res) => {
    try {
        ...
        const rate = await exchangeRateRepo
            .createQueryBuilder('rate')
            .where('rate.fromCurrency = :currency', { currency: fromCurrency.toUpperCase() })
...
router.get('/convert', async (req, res) => {
    try {
        ...
        const result = await currency_service_1.CurrencyService.convertToMYR(parseFloat(amount), fromCurrency.toUpperCase());
        res.json(result);
    }
    catch (error) {
...
//# sourceMappingURL=exchangeRate.routes.js.map