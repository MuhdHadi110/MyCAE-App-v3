 ...
 */
router.post('/', [
    (0, express_validator_1.body)('company_id').notEmpty().withMessage('Company ID is required'),
    ...
    (0, express_validator_1.body)('position').optional().isString(),
], async (req, res) => {
    try {
        ...
        const existingContact = await contactRepo.findOne({
            where: { company_id, email, deleted_at: null },
        });
        if (existingContact) {
 ...
 */
router.put('/:id', [
    (0, express_validator_1.body)('name').optional().notEmpty().withMessage('Contact name cannot be empty'),
    ...
    try {
        ...
        if (email && email !== contact.email) {
            const existingContact = await contactRepo.findOne({
                where: { company_id: contact.company_id, email, deleted_at: null },
            });
            if (existingContact) {
                return res.status(400).json({ error: 'Contact with this email already exists for this company' });
            }
