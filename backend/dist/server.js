...
const projectTeam_routes_1 = __importDefault(require("./routes/projectTeam.routes"));
...
const validateProductionEnv = () => {
    ...
    if (missing.length > 0) {
        logger_1.logger.error('FATAL: Missing required environment variables in production', { missing });
        process.exit(1);
    }
    // Validate JWT_SECRET strength
    const jwtSecret = process.env.JWT_SECRET;
    if (jwtSecret && jwtSecret.length < 32) {
        logger_1.logger.error('FATAL: JWT_SECRET must be at least 32 characters long for security', { length: jwtSecret.length });
        process.exit(1);
    }
...
const getAllowedOrigins = () => {
    ...
    if (isProduction) {
        if (!process.env.CORS_ORIGINS) {
            logger_1.logger.error('FATAL: CORS_ORIGINS environment variable is required in production');
            process.exit(1);
        }
...
app.use('/api/auth', authLimiter, auth_routes_1.default); // Apply strict rate limiting to auth routes
app.use('/api/', apiLimiter); // Apply to all other API routes
app.use('/api/users', users_routes_1.default);
app.use('/api/inventory', inventory_routes_1.default);
app.use('/api/checkouts', checkout_routes_1.default);
app.use('/api/projects', projectTeam_routes_1.default); // Project team management routes - MUST be before projectRoutes
...
app.use((err, req, res, next) => {
    logger_1.logger.error('Unhandled error', { error: err.message, path: req.path });
...
const gracefulShutdown = async (signal) => {
    ...
    try {
        ...
        logger_1.logger.info('Stopping scheduled tasks...');
