...
router.get('/team-assignments', async (req, res) => {
    try {
        logger_1.logger.info('Fetching team assignments from timesheets, project_team_members, and projects');
        ...
        const timesheetAssignments = await database_1.AppDataSource
            .createQueryBuilder()
            ...
            .getRawMany();
        logger_1.logger.info(`Found ${timesheetAssignments.length} assignments from timesheets`);
        // Secondary source: Get assignments from project_team_members table
        const teamMemberAssignments = await database_1.AppDataSource
            .createQueryBuilder()
            ...
            .getRawMany();
        logger_1.logger.info(`Found ${teamMemberAssignments.length} assignments from project_team_members`);
        ...
        const projectsWithLeads = await database_1.AppDataSource
            .createQueryBuilder()
            ...
            .getRawMany();
        logger_1.logger.info(`Found ${projectsWithLeads.length} projects with lead engineers`);
        // Group by engineer ID
        const groupedByMember = {};
        // Helper function to add assignment
        const addAssignment = (engineerId, projectData, role) => {
            if (!groupedByMember[engineerId]) {
                groupedByMember[engineerId] = [];
            }
            // Check if already added (avoid duplicates)
            const existing = groupedByMember[engineerId].find((a) => a.projectId === projectData.projectId);
            if (!existing) {
        ...
        timesheetAssignments.forEach((assignment) => {
            if (!assignment.teamMemberId) {
                logger_1.logger.warn(`Timesheet assignment has no matching team member: project_id=${assignment.projectId}`);
                return;
            }
        ...
        projectsWithLeads.forEach((project) => {
            if (!project.teamMemberId) {
                logger_1.logger.warn(`Lead engineer assignment has no matching team member: project_id=${project.projectId}`);
                return;
            }
            addAssignment(project.teamMemberId, project, 'lead_engineer');
        });
        logger_1.logger.info(`Grouped assignments for ${Object.keys(groupedByMember).length} members total`);
        res.json(groupedByMember);
    }
    catch (error) {
        logger_1.logger.error('Error fetching team assignments', { error });
...
router.get('/:projectId/team', async (req, res) => {
    ...
    try {
        const teamMembers = await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember)
        ...
        logger_1.logger.error('Error fetching project team', { error, projectId });
 ...
 */
router.post('/:projectId/team', (0, auth_1.authorize)(User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    const { projectId } = req.params;
    const { teamMemberId, role = ProjectTeamMember_1.ProjectTeamRole.ENGINEER } = req.body;
    try {
        if (!teamMemberId) {
            return res.status(400).json({ error: 'Team member ID is required' });
        }
        // Check if project exists
        const project = await database_1.AppDataSource.getRepository(Project_1.Project).findOne({
            where: { id: projectId },
        });
        if (!project) {
        ...
        const existing = await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember).findOne({
        ...
        let newMemberId;
        try {
            const savedMember = await database_1.AppDataSource.transaction(async (transactionalEntityManager) => {
                // If assigning as lead, check if lead already exists (within transaction)
                if (role === ProjectTeamMember_1.ProjectTeamRole.LEAD_ENGINEER) {
                    const existingLead = await transactionalEntityManager.findOne(ProjectTeamMember_1.ProjectTeamMember, {
                        where: { project_id: projectId, role: ProjectTeamMember_1.ProjectTeamRole.LEAD_ENGINEER },
                    });
                    if (existingLead) {
                ...
                const projectTeamMember = transactionalEntityManager.create(ProjectTeamMember_1.ProjectTeamMember, {
        ...
        const savedMemberWithRelations = await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember)
            .createQueryBuilder('ptm')
            .leftJoinAndSelect('ptm.teamMember', 'tm')
            .leftJoinAndSelect('tm.user', 'u')
        ...
        logger_1.logger.error('Error adding team member', { error, projectId, teamMemberId });
 ...
 */
router.put('/:projectId/team/:teamMemberId', (0, auth_1.authorize)(User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    const { projectId, teamMemberId } = req.params;
    try {
        const { role } = req.body;
        const projectTeamMember = await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember).findOne({
            where: { project_id: projectId, team_member_id: teamMemberId },
        });
        ...
        if (role === ProjectTeamMember_1.ProjectTeamRole.LEAD_ENGINEER) {
            const existingLead = await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember).findOne({
                where: { project_id: projectId, role: ProjectTeamMember_1.ProjectTeamRole.LEAD_ENGINEER },
            });
            if (existingLead && existingLead.id !== projectTeamMember.id) {
                existingLead.role = ProjectTeamMember_1.ProjectTeamRole.ENGINEER;
                await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember).save(existingLead);
            }
        }
        projectTeamMember.role = role;
        await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember).save(projectTeamMember);
        res.json({
        ...
        logger_1.logger.error('Error updating team member', { error, projectId, teamMemberId });
 ...
 */
router.delete('/:projectId/team/:teamMemberId', (0, auth_1.authorize)(User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    const { projectId, teamMemberId } = req.params;
    try {
        const result = await database_1.AppDataSource.getRepository(ProjectTeamMember_1.ProjectTeamMember).delete({
            project_id: projectId,
            team_member_id: teamMemberId,
        });
        ...
        logger_1.logger.error('Error removing team member', { error, projectId, teamMemberId });
        res.status(500).json({ error: 'Failed to remove team member' });
    }
});
exports.default = router;
//# sourceMappingURL=projectTeam.routes.js.map