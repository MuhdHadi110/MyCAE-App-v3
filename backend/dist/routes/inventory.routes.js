...
router.get('/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching item', { error });
...
router.put('/:id', (0, auth_1.authorize)(...INVENTORY_MODIFY_ROLES), async (req, res) => {
    try {
        ...
        if (oldStatus !== InventoryItem_1.InventoryStatus.LOW_STOCK &&
            ...
            n8n_service_1.default.onLowStockAlert({
                itemId: item.id,
                itemName: item.title,
                sku: item.sku,
                ...
                logger_1.logger.warn('n8n low stock alert error (non-blocking)', { error: err.message });
            });
...
router.delete('/:id', (0, auth_1.authorize)(...INVENTORY_MODIFY_ROLES), async (req, res) => {
    try {
        ...
        logger_1.logger.debug('Deleted checkout records', { itemId });
        // Then delete the inventory item
        await inventoryRepo.remove(item);
        logger_1.logger.info('Item deleted successfully', { itemId });
...
router.post('/bulk/create', (0, auth_1.authorize)(...INVENTORY_MODIFY_ROLES), async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error bulk creating items', { error });
