...
router.post('/single', [
    ...
    try {
        ...
        n8n_service_1.default.onCheckoutCreated({
            ...
            logger_1.logger.warn('n8n webhook error (non-blocking)', { error: err.message });
        });
        res.status(201).json({
            success: true,
            checkoutId: checkout.id,
        ...
        logger_1.logger.error('Error creating single checkout', { error });
    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                const updateResult = await queryRunner.manager.decrement(InventoryItem_1.InventoryItem, { id: inventoryItem.id }, 'quantity', itemData.quantity);
                ...
                const updatedItem = await queryRunner.manager.findOne(InventoryItem_1.InventoryItem, {
        ...
        logger_1.logger.error('Error creating bulk checkout', { error });
...
router.post('/checkin/single', [
    ...
    try {
        ...
        logger_1.logger.error('Error checking in single item', { error });
...
router.post('/checkin/bulk', [(0, express_validator_1.body)('masterBarcode').notEmpty().withMessage('Master barcode is required')], async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error checking in bulk items', { error });
...
router.get('/', async (req, res) => {
    try {
        ...
        checkouts.forEach((checkout) => {
            ...
            if (checkout.status === Checkout_1.CheckoutStatus.RECEIVED) {
        ...
        logger_1.logger.error('Error fetching checkouts', { error });
...
router.get('/:masterBarcode', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching checkout', { error });
