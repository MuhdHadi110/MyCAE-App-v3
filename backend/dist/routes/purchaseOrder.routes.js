...
router.get('/', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching purchase orders', { error });
...
router.get('/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching purchase order', { error });
    ...
    try {
        ...
        try {
            ...
            if (mds.length > 0) {
                ...
                await email_service_1.default.sendPOReceivedNotification(po.id, mds, poNumber, parseFloat(amount), currency, projectCode, clientName, req.user.name, req.user.email, receivedDateFormatted, receivedTimeFormatted);
            }
        }
        catch (emailError) {
        ...
        logger_1.logger.error('Error deleting purchase order', { error });
...
router.put('/:id', (0, auth_1.authorize)(User_1.UserRole.SENIOR_ENGINEER, User_1.UserRole.PRINCIPAL_ENGINEER, User_1.UserRole.MANAGER, User_1.UserRole.MANAGING_DIRECTOR, User_1.UserRole.ADMIN), async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error updating purchase order', { error });
...
router.get('/:poNumberBase/revisions', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error creating PO revision', { error });
...
router.post('/:id/revisions', [
    ...
    try {
        ...
        logger_1.logger.error('Failed to send PO notification', { error: emailError.message });
    }
});
/**
...
router.patch('/:id/adjust-myr', [
    ...
    try {
        ...
        logger_1.logger.error('Error adjusting MYR amount', { error });
