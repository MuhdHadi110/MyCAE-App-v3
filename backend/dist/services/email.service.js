...
class EmailService {
    ...
    async verifyConnection() {
        try {
            ...
            logger_1.logger.info('SMTP Server connection verified');
        }
        catch (error) {
            logger_1.logger.error('SMTP Connection Error', { error: error.message });
            logger_1.logger.warn('Email notifications will not work until SMTP is configured');
        }
    }
    /**
    ...
    async sendEmail(to, subject, html) {
        try {
            ...
            logger_1.logger.debug('Email sent successfully', { recipient: to, subject });
        }
        catch (error) {
            logger_1.logger.error('Failed to send email', { recipient: to, error: error.message });
            throw error;
        }
    }
    /**
     ...
     */
    async sendCheckoutNotification(userEmail, userName, itemDetails, expectedReturnDate) {
        const subject = 'Equipment Checkout Confirmation';
        const html = this.getEmailTemplate(userName, `Your equipment checkout has been confirmed.`, `
     ...
     */
    async sendReturnReminder(userEmail, userName, itemDetails, dueDate) {
        const subject = 'Equipment Return Reminder';
        const html = this.getEmailTemplate(userName, `This is a reminder to return your checked-out equipment.`, `
     ...
     */
    async sendLowStockAlert(adminEmail, itemName, currentStock, minStock) {
        const subject = `Low Stock Alert: ${itemName}`;
        const html = this.getEmailTemplate('Admin', `Low stock alert for ${itemName}`, `
     ...
     */
    async sendMaintenanceTicketNotification(assigneeEmail, assigneeName, ticketTitle, priority) {
        const subject = `New Maintenance Ticket Assigned: ${ticketTitle}`;
        const html = this.getEmailTemplate(assigneeName, `A new maintenance ticket has been assigned to you.`, `
     ...
     */
    async sendProjectAssignmentNotification(userEmail, userName, projectTitle, projectCode, role, companyName, contactName, contactEmail, managerName) {
        const subject = `New Project Assignment: ${projectTitle}`;
        const html = this.getProjectAssignmentTemplate(userName, projectTitle, projectCode, role, companyName, contactName, contactEmail, managerName);
        await this.sendEmail(userEmail, subject, html);
    }
    /**
     * Project assignment email template - Clean professional format
     */
    getProjectAssignmentTemplate(userName, projectTitle, projectCode, role, companyName, contactName, contactEmail, managerName) {
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
     ...
     */
    async sendMaintenanceReminder(recipientEmail, recipientName, itemName, maintenanceType, scheduledDate, timeframe) {
        ...
        const html = this.getMaintenanceReminderTemplate(recipientName, itemName, maintenanceType, formattedDate, timeframe);
        await this.sendEmail(recipientEmail, subject, html);
    }
    /**
     * Send overdue maintenance alert
     */
    async sendMaintenanceOverdueAlert(recipientEmail, recipientName, itemName, maintenanceType, scheduledDate, daysOverdue) {
        const formattedDate = scheduledDate.toLocaleDateString('en-MY', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
        ...
        const html = this.getMaintenanceOverdueTemplate(recipientName, itemName, maintenanceType, formattedDate, daysOverdue);
        await this.sendEmail(recipientEmail, subject, html);
    }
    /**
     * Send maintenance completed notification
     */
    async sendMaintenanceCompletedNotification(recipientEmail, recipientName, itemName, maintenanceType, completedBy) {
        const subject = `Maintenance Completed: ${itemName} - ${maintenanceType}`;
        const html = this.getEmailTemplate(recipientName, `The scheduled maintenance has been completed.`, `
        <p><strong>Item:</strong> ${itemName}</p>
     ...
     */
    getMaintenanceReminderTemplate(userName, itemName, maintenanceType, scheduledDate, timeframe) {
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
     ...
     */
    getMaintenanceOverdueTemplate(userName, itemName, maintenanceType, scheduledDate, daysOverdue) {
        const supportEmail = process.env.SUPPORT_EMAIL || 'noreply@mycaetech.com';
        return `
<!DOCTYPE html>
<html>
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
     ...
     */
    getEmailTemplate(userName, message, content, actionUrl) {
        const supportEmail = process.env.SUPPORT_EMAIL || 'noreply@mycaetech.com';
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
    ...
    async sendInvoiceApprovalRequest(invoiceId, mds, invoiceNumber, invoiceAmount, currency, projectCode, projectName, creatorName, creatorEmail, submittedDate, submittedTime) {
        ...
        for (const md of mds) {
            try {
                const personalizedHtml = this.getInvoiceApprovalRequestTemplate(invoiceNumber, invoiceAmount, currency, projectCode, projectName, creatorName, creatorEmail, submittedDate, submittedTime, approvalUrl, md.name);
                await this.sendEmail(md.email, subject, personalizedHtml);
                logger_1.logger.debug('Invoice approval email sent', { recipient: md.name, email: md.email });
            }
            catch (error) {
                logger_1.logger.error('Failed to send invoice approval email', { recipient: md.name, error: error.message });
            }
        }
    }
    /**
     ...
     */
    async sendPOReceivedNotification(poId, mds, poNumber, poAmount, currency, projectCode, clientName, creatorName, creatorEmail, receivedDate, receivedTime) {
        ...
        for (const md of mds) {
            try {
                const personalizedHtml = this.getPOReceivedNotificationTemplate(poNumber, poAmount, currency, projectCode, clientName, creatorName, creatorEmail, receivedDate, receivedTime, poUrl, md.name);
                await this.sendEmail(md.email, subject, personalizedHtml);
                logger_1.logger.debug('PO notification sent', { recipient: md.name, email: md.email });
            }
            catch (error) {
                logger_1.logger.error('Failed to send PO notification', { recipient: md.name, error: error.message });
            }
        }
    }
    /**
     ...
     */
    getPOReceivedNotificationTemplate(poNumber, poAmount, currency, projectCode, clientName, creatorName, creatorEmail, receivedDate, receivedTime, poUrl, mdName) {
        return `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
     ...
     */
    getInvoiceApprovalRequestTemplate(invoiceNumber, invoiceAmount, currency, projectCode, projectName, creatorName, creatorEmail, submittedDate, submittedTime, approvalUrl, mdName) {
        return `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
    ...
    async sendInvoiceApprovalConfirmation(invoiceId, creatorEmail, creatorName, invoiceNumber, invoiceAmount, currency, projectCode, projectName, approverName, approvedDate, approvedTime) {
        ...
        const html = this.getInvoiceApprovalConfirmationTemplate(creatorName, invoiceNumber, invoiceAmount, currency, projectCode, projectName, approverName, approvedDate, approvedTime, invoiceUrl);
        await this.sendEmail(creatorEmail, subject, html);
        logger_1.logger.debug('Invoice approval confirmation sent', { recipient: creatorName, email: creatorEmail });
    }
    /**
     * Invoice approval confirmation email template - Clean professional format
     */
    getInvoiceApprovalConfirmationTemplate(creatorName, invoiceNumber, invoiceAmount, currency, projectCode, projectName, approverName, approvedDate, approvedTime, invoiceUrl) {
        return `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
    ...
    async sendInvoiceWithdrawnNotification(invoiceId, mds, invoiceNumber, invoiceAmount, currency, projectCode, projectName, creatorName, withdrawnDate, withdrawnTime) {
        const subject = `Invoice Withdrawn: ${invoiceNumber} - ${currency} ${invoiceAmount.toLocaleString()}`;
        // Send to all MDs
        for (const md of mds) {
            try {
                const personalizedHtml = this.getInvoiceWithdrawnTemplate(invoiceNumber, invoiceAmount, currency, projectCode, projectName, creatorName, withdrawnDate, withdrawnTime, md.name);
                await this.sendEmail(md.email, subject, personalizedHtml);
                logger_1.logger.debug('Invoice withdrawn notification sent', { recipient: md.name, email: md.email });
            }
            catch (error) {
                logger_1.logger.error('Failed to send invoice withdrawn notification', { recipient: md.name, error: error.message });
            }
        }
    }
    /**
     ...
     */
    getInvoiceWithdrawnTemplate(invoiceNumber, invoiceAmount, currency, projectCode, projectName, creatorName, withdrawnDate, withdrawnTime, mdName) {
        return `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Arial, sans-serif; font-size: 14px; line-height: 1.6; color: #333333; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto;">
     ...
     */
    async sendWelcomeEmail(userEmail, userName, tempPassword, expiryDate, pdfBuffer) {
...
MyCAE Admin Team`;
        try {
            ...
            logger_1.logger.info('Welcome email sent', { email: userEmail });
        }
        catch (error) {
            logger_1.logger.error('Failed to send welcome email', { email: userEmail, error: error.message });
            throw error;
        }
    }
}
