...
export class TeamService {
   ...
   */
  async calculateWorkloadData(userIds: string[]): Promise<Record<string, WorkloadData>> {
   ...
   */
  async getTeamMembers(filters: TeamMemberFilters = {}) {
   ...
   * Update team member
   */
  async updateTeamMember(id: string, data: UpdateTeamMemberData, currentUserId?: string, currentUserRole?: string) {
    const member = await this.teamRepo.findOne({
      where: { id },
      relations: ['user'],
    });
    ...
    if (rolesToUpdate) {
      ...
      user.roles = rolesArray as UserRole[];
      userUpdated = true;
    }
    if (avatar) {
   ...
   */
  async getTeamMembersByDepartment(department: string, filters: { limit?: number; offset?: number } = {}) {
