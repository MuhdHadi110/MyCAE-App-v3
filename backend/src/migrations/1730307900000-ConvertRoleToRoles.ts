...
export class ConvertRoleToRoles1730307900000 implements MigrationInterface {
  name = 'ConvertRoleToRoles1730307900000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!hasRolesColumn) {
      await queryRunner.query(
        `ALTER TABLE users ADD COLUMN roles VARCHAR(255) NOT NULL DEFAULT 'engineer'`
      );
    }

    // Copy data from 'role' to 'roles' - wrap single role in array
    await queryRunner.query(
      `UPDATE users SET roles = CONCAT(role) WHERE roles = 'engineer'`
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE users SET role = SUBSTRING_INDEX(roles, ',', 1) WHERE role = 'engineer'`
    );
