...
const router = Router();
const projectHourlyRateRepository = AppDataSource.getRepository(ProjectHourlyRate);
  ...
  try {
    ...
    res.json({
      ...
      data: rates.map(rate => ({
        ...
        hourlyRate: parseFloat(rate.hourlyRate as any),
...
router.put('/:projectId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const [teamMemberId, hourlyRate] of Object.entries(rates)) {
      const rate = parseFloat(hourlyRate as any);
...
router.get('/rate/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
// Delete hourly rate
router.delete('/:projectId/:teamMemberId', authenticate, async (req: AuthRequest, res: Response) => {
  try {
