 ...
 */
router.get('/', authorize(UserRole.SENIOR_ENGINEER, UserRole.PRINCIPAL_ENGINEER, UserRole.MANAGER, UserRole.MANAGING_DIRECTOR, UserRole.ADMIN), async (req: AuthRequest, res: Response) => {
  try {
...
router.delete('/:id',
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const cumulativePercentage = remainingInvoices.reduce(
        (sum, inv) => sum + Number(inv.percentage_of_total),
        0
      );
      ...
      if (cumulativePercentage < 100) {
        ...
        if (project && project.status === ProjectStatus.COMPLETED) {
          logger.info(`Reverting project ${primaryProjectCode} status from completed to ongoing (invoice deleted, cumulative: ${cumulativePercentage}%)`);
          project.status = ProjectStatus.ONGOING;
          project.completion_date = undefined;
          await projectRepo.save(project);
          logger.info(`Project ${primaryProjectCode} status reverted to ongoing`);
          statusReverted = true;
        }
      }
  ...
  try {
    ...
    if (invoice.file_url) {
      ...
      if (invoice.file_url.includes('/uploads/')) {
        ...
        filename = path.basename(pathAfterUploads);
      }
