...
interface InvoiceExcelRow {
  projectNumberAndName: string;
  invoiceNumber: string;
  rmValueExclTax: string;
  invoiceDate: string;
...
function parsePercentage(pctStr: string): number {
  if (!pctStr) return 100; // Default to 100% if not specified
 ...
 */
function extractProjectCode(projectStr: string): string {
...
async function importInvoices() {
  ...
  try {
    ...
    const projectInvoices: { [key: string]: Invoice[] } = {};
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      try {
        ...
        invoice.status = InvoiceStatus.SENT; // Assume existing invoices are sent
